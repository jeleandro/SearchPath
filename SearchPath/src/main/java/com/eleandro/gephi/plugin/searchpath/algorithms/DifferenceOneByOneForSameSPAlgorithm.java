 ...
 * @author eleandro
 */
public class DifferenceOneByOneForSameSPAlgorithm extends DifferenceBetweenPathsAlgorithm {

    private float minDiff;

    public DifferenceOneByOneForSameSPAlgorithm(float percMinimumDiffBetweenPaths) {
    ...
    public void execute(List<Path> paths) {
        ...
        if (paths.size() > 2) {
            for (int i = 0; i < paths.size() - 1;) {
                int relI = paths.get(i).getRelevance();
                int relI1 = paths.get(i + 1).getRelevance();
                if (relI == relI1) {
